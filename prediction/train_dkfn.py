...
def TrainDKFN(train_dataloader, valid_dataloader, A, K=3, num_epochs=1):
    ...
    learning_rate = 1e-4
    dkfn = DKFN(att_size, step_size, fea_size, K, torch.Tensor(A), pred_size)
    dkfn.to(DEVICE)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1)%10 == 0:
            torch.save(dkfn.state_dict(), './models/dkfn_'+str(epoch))

    np.save('loss_dkfn.npy', np.array(losses_epoch), allow_pickle=True)
