...
def TrainneDKFN(train_dataloader, valid_dataloader, A, K=3, num_epochs=1):
    ...
    learning_rate = 1e-4
    nedkfn = neDKFN(att_size, step_size, fea_size, K, torch.Tensor(A), pred_size)
    nedkfn.to(DEVICE)
    optimizer = torch.optim.RMSprop(nedkfn.parameters(), lr=learning_rate)

    ###################### print model parameter states ######################
    print('Net\'s state_dict:')
    total_param = 0
    for param_tensor in nedkfn.state_dict():
        print(param_tensor, '\t', nedkfn.state_dict()[param_tensor].size())
        total_param += np.prod(nedkfn.state_dict()[param_tensor].size())
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_dataloader:
            ...
            torch.nn.utils.clip_grad_norm_(nedkfn.parameters(), clipping_value)
            optimizer.step()
            ...
            pred = nedkfn(atts_val, cases_val)
    ...
    np.save('loss_nedkfn.npy', np.array(losses_epoch), allow_pickle=True)
    return nedkfn, [losses_train, losses_interval_train, losses_valid, losses_interval_valid]


def TestneDKFN(nedkfn, test_dataloader, max_speed):
    nedkfn.eval()
